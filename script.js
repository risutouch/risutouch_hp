...
// ── ヒーロー装飾パーツ（ランダム配置・アニメーション）──
(function () {
  const container = document.getElementById('hero-decos');
  if (!container) return;

  const decos = [
    { src: 'images/deco_tree.png',   h: 110, anim: 'sway' },
    { src: 'images/deco_pine.png',   h: 105, anim: 'sway' },
    { src: 'images/deco_grass.png',  h:  55, anim: 'sway' },
    { src: 'images/deco_flower.png', h:  70, anim: 'float' },
    { src: 'images/deco_rock.png',   h:  48, anim: 'float' },
  ];

  // 5 horizontal zones (% from left), shuffled so each run differs
  const zones = [4, 18, 68, 80, 92];
  ...
  decos.forEach((deco, i) => {
    ...
    img.src = deco.src;
    img.alt = '';
    img.className = 'hero-deco hero-deco--' + deco.anim;

    const jitter = (Math.random() * 6 - 3).toFixed(1);
    const delay  = (Math.random() * 2.5).toFixed(2) + 's';
    const bottom = deco.h < 60
      ? (58 + Math.random() * 10).toFixed(0) + 'px'
      : (50 + Math.random() * 20).toFixed(0) + 'px';

    img.style.cssText =
      `height:${deco.h}px;` +
      `left:calc(${zones[i]}% + ${jitter}%);` +
      `bottom:${bottom};` +
