...
// メール設定（用途別に分離）
$ADMIN_LOGIN_EMAIL = 'risutouch@gmail.com';  // 管理者ログイン用（認証先）
$SYSTEM_FROM_EMAIL = 'info@risutouch.com';   // システム送信元メール（ConoHa SMTP）
...
";
    ...
    $headers = "From: " . $SYSTEM_FROM_EMAIL . "\r\n";
    $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
    $headers .= "Reply-To: " . $SYSTEM_FROM_EMAIL . "\r\n";
    
    // ログに出力（開発・デバッグ用）
    error_log("=== パスワードリセットメール送信 ===");
    error_log("Password reset email requested");
...
";
    ...
    // ログに出力（デバッグ用）
    error_log("Email verification code requested for login");
...
function sendViaConoHaSMTP($email, $subject, $message) {
    global $SYSTEM_FROM_EMAIL;
    
    // ConoHaメールサーバーのSMTP設定
    error_log("ConoHa SMTP設定開始: mail37.conoha.ne.jp:587");
    error_log("送信元メール: " . $SYSTEM_FROM_EMAIL);
    error_log("送信先メール: " . $email);
    ini_set('SMTP', 'mail37.conoha.ne.jp');
    ini_set('smtp_port', '587'); // または '25'
    ini_set('sendmail_from', $SYSTEM_FROM_EMAIL);
    
    $headers = "From: " . $SYSTEM_FROM_EMAIL . "\r\n";
    $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
    $headers .= "Reply-To: " . $SYSTEM_FROM_EMAIL . "\r\n";
    
    error_log("Attempting ConoHa SMTP mail delivery");
    ...
    elseif (password_verify($_POST['password'], getCurrentPasswordHash())) {
        ...
        if (DEVELOPMENT_MODE) {
            ...
            if ($sent) {
                ...
                error_log("Email verification code sent successfully");
                ...
                error_log("Email sending failed - displaying code for emergency access");
...
if (isset($_POST['verify_email_code'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        recordLoginAttempt($client_ip, false);
        error_log("Email verification failed - invalid code");
    } else {
        // メール認証成功 - ログイン完了
        ...
        recordLoginAttempt($client_ip, true);
        error_log("Email verification successful - admin login completed");
...
if ($isLoggedIn) {
    ...
    if (isset($_POST['save_settings'])) {
        ...
        if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
            $_SESSION['message'] = 'セキュリティエラー: 無効なリクエストです';
            header('Location: index.php?tab=settings');
            exit;
        }
        
        $result = saveSettings($_POST, $_FILES);
        $message = $result['success'] ? '設定を保存しました' : '保存に失敗しました: ' . $result['error'];
...
// ファイルアップロード関数
function uploadFile($file, $uploadDir, $baseName) {
    // ディレクトリが存在しない場合は作成
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0755, true);
    }
...
// 基本設定保存関数
function saveSettings($data, $files) {
    try {
        // デバッグ用ログ
        error_log("saveSettings called with data: " . print_r($data, true));
        ...
        // POSTデータから設定を更新
        if (isset($data['site'])) {
            $config['site'] = array_merge($config['site'] ?? [], $data['site']);
        }
        if (isset($data['contact'])) {
            $config['contact'] = array_merge($config['contact'] ?? [], $data['contact']);
        }
        if (isset($data['seo'])) {
            $config['seo'] = array_merge($config['seo'] ?? [], $data['seo']);
        }
        if (isset($data['manifest'])) {
            $config['manifest'] = array_merge($config['manifest'] ?? [], $data['manifest']);
        }
        
        // Favicon設定
        $config['favicon'] = [
            'enabled' => isset($data['favicon']['enabled']) ? true : false,
            'favicon_ico' => $config['favicon']['favicon_ico'] ?? 'assets/favicons/favicon.ico',
            'favicon_png' => $config['favicon']['favicon_png'] ?? 'assets/favicons/favicon.png',
            'apple_touch_icon' => $config['favicon']['apple_touch_icon'] ?? 'assets/favicons/apple-touch-icon.png'
        ];
        ...
        if (isset($files['og_image']) && $files['og_image']['error'] === UPLOAD_ERR_OK) {
            $ogImagePath = uploadFile($files['og_image'], $uploadDir, 'og-image');
            if ($ogImagePath) {
                $config['seo']['og_image'] = $ogImagePath;
            }
        }
        
        // Faviconファイルのアップロード
        if (isset($files['favicon_file']) && $files['favicon_file']['error'] === UPLOAD_ERR_OK) {
            $faviconPath = uploadFile($files['favicon_file'], $faviconDir, 'favicon');
            if ($faviconPath) {
                $config['favicon']['favicon_png'] = $faviconPath;
                // ICOファイルとしてもコピー
                $icoPath = str_replace('.png', '.ico', $faviconPath);
                copy('../' . $faviconPath, '../' . $icoPath);
                $config['favicon']['favicon_ico'] = $icoPath;
        ...
        // Apple Touch Iconのアップロード
        if (isset($files['apple_icon_file']) && $files['apple_icon_file']['error'] === UPLOAD_ERR_OK) {
            $applePath = uploadFile($files['apple_icon_file'], $faviconDir, 'apple-touch-icon');
            if ($applePath) {
                $config['favicon']['apple_touch_icon'] = $applePath;
            }
        }
        ...
        if ($jsonData === false) {
            ...
            return ['success' => false, 'error' => 'JSONエンコードエラー: ' . json_last_error_msg()];
        }
        
        // ディレクトリの書き込み権限をチェック
        $configDir = dirname($configFile);
        if (!is_writable($configDir)) {
            error_log("Directory not writable: $configDir");
            return ['success' => false, 'error' => '設定ディレクトリに書き込み権限がありません'];
        }
        
        if (file_put_contents($configFile, $jsonData)) {
            // manifest.jsonを更新
            updateManifestFile($config);
            ...
            return ['success' => false, 'error' => '設定ファイルの保存に失敗しました'];
        }
...
function updateManifestFile($config) {
    $manifest = [
        'name' => $config['manifest']['name'] ?? '焼き菓子.りすたっち',
        'short_name' => $config['manifest']['short_name'] ?? 'りすたっち',
        'description' => $config['site']['description'] ?? '',
        'start_url' => '/',
        'display' => $config['manifest']['display'] ?? 'standalone',
        'background_color' => $config['site']['background_color'] ?? '#ffffff',
        'theme_color' => $config['site']['theme_color'] ?? '#4a3c2a',
        'icons' => [
            [
                'src' => $config['favicon']['favicon_png'] ?? 'assets/favicons/favicon.png',
                'sizes' => 'any',
                'type' => 'image/png'
            ],
            [
                'src' => $config['favicon']['apple_touch_icon'] ?? 'assets/favicons/apple-touch-icon.png',
                'sizes' => '180x180',
                'type' => 'image/png'
            ]
        ]
...
function updateHtmlMetaTags($config) {
    ...
    // Google Search Console認証タグの更新
    if (!empty($config['seo']['google_verification'])) {
        $googleTag = '<meta name="google-site-verification" content="' . htmlspecialchars($config['seo']['google_verification']) . '">';
        
        // 既存のGoogle認証タグを探して置換、なければコメント後に挿入
        if (preg_match('/<meta name="google-site-verification"[^>]*>/', $html)) {
            $html = preg_replace('/<meta name="google-site-verification"[^>]*>/', $googleTag, $html);
        } else {
            $html = str_replace(
        ...
        // 認証コードが空の場合、既存のタグを削除してコメントに戻す
        $html = preg_replace('/<meta name="google-site-verification"[^>]*>\s*/', '<!-- google-site-verification メタタグは設定で追加されます -->' . "\n    ", $html);
    }
                    ...
                    <form method="POST" class="login-form">
                        ...
                        <?php if (DEVELOPMENT_MODE): ?>
                            <div class="alert alert-info" style="margin-bottom: 20px;">
                                <strong>🛠️ 開発モード</strong><br>
                                メール認証がバイパスされます。パスワードのみでログインできます。
                            </div>
                        <?php endif; ?>
                        <div class="form-group">
                            <label for="email">メールアドレス</label>
    ...
    <?php else: ?>
        ...
        <div class="admin-panel">
            ...
            <nav class="admin-nav">
                ...
                <a href="?tab=settings" class="nav-btn <?php echo $currentTab === 'settings' ? 'active' : ''; ?>">基本設定</a>
