    ...
    let tl = gsap.timeline({
        scrollTrigger: {
            ...
            onUpdate: (self) => {
                ...
                console.log('Progress:', progress, 'Panel index:', panelIndex);
...
const productsPerPage = 3; // 1回に表示する商品数
...
function updateLoadMoreButton() {
    ...
    if (!loadMoreContainer) {
        ...
        if (productsShowcase) {
            ...
            console.error('Products showcase container not found');
            return;
        }
    }
...
async function loadTopics() {
    try {
        ...
        console.log('トピック設定を読み込みました:', data);
        displayTopics(data);
    } catch (error) {
        console.error('トピック設定の読み込みに失敗しました:', error);
        // デフォルト設定で表示（全て表示）
        displayTopics({});
    }
}
...
async function loadMenu() {
    try {
        ...
        console.warn('メニュー情報の読み込みに失敗:', error);
        // フォールバック - 既存の画像パスを使用
...
function showMenuButton(menuName) {
    ...
    if (!menuDisplay) {
        console.error('menu-display要素が見つかりません！');
        return;
    }
                ...
                try {
                    ...
                    setTimeout(() => {
                        if (typeof startQuiz === 'function') {
                            console.log('startQuiz function is now available');
                        }
                    }, 100);
                } catch (error) {
...
function initEasterEgg() {
    ...
    console.log('Easter egg initialized on logo image');
    ...
    logoImage.addEventListener('dblclick', function(e) {
        ...
        console.log('Logo double click detected, creating acorn at center:', centerX, centerY);
        createAcornAtPosition(centerX, centerY);
    });
    ...
    logoImage.addEventListener('touchend', function(e) {
        ...
        if (timeDiff < 400 && timeDiff > 50 && !isDoubleTapProcessed) {
            ...
            if (tapCount === 2) {
                ...
                console.log('Logo double tap detected, creating acorn at center:', centerX, centerY);
                createAcornAtPosition(centerX, centerY);
                tapCount = 0;
            }
        } else {
...
function startPhysicsLoop() {
    ...
    console.log('Starting physics loop, physics acorns:', physicsAcorns.length);
    ...
    function updatePhysics() {
        ...
        acorns.forEach((acorn, index) => {
            ...
            if (index === 0 && acorn.physics.y < 100) {
                console.log(`Physics update: x=${acorn.physics.x.toFixed(1)}, y=${acorn.physics.y.toFixed(1)}, transform=${transform}`);
            }
        });
        ...
        if (physicsAcorns.length > 0) {
            ...
            console.log('No physics acorns, stopping loop');
...
function createAcornAtPosition(x, y) {
    console.log('Creating acorn at position:', x, y);
    
    if (!easterEggActive) {
        easterEggActive = true;
        console.log('🌰 Physics Acorn Easter Egg Activated! 🌰');
    }
    ...
    img.onerror = function() {
        console.log('Acorn image load failed, using fallback');
    ...
    // サイズとスタイル
    const size = 40 + Math.random() * 20; // 40px-60px
    ...
    console.log('Acorn initial transform:', `translate(${x - size/2}px, ${y - size/2}px) rotate(${rotation}deg) scale(0)`);
    
    document.body.appendChild(acorn);
    console.log('Acorn added to DOM at:', x, y);
    
    // 物理演算プロパティを追加
    acorn.physics = {
        x: x,
        y: y,
        velocityX: (Math.random() - 0.5) * 4, // -2 から 2 の初速度
        velocityY: -8 - Math.random() * 4, // 上向きの初速度（ふわっと浮く）
        size: size,
        rotation: rotation
    };
    ...
    setTimeout(() => {
        ...
        console.log('Acorn appearance animation:', newTransform);
    }, 10);
    
    // 配列に追加
    acorns.push(acorn);
    console.log('Acorn added to array, total:', acorns.length);
    
    // 物理演算が動いていなければ開始
    if (!animationFrame) {
        console.log('Starting physics for new acorn, current acorns:', acorns.length);
        startPhysicsLoop();
    }
    ...
    setTimeout(() => {
        if (acorn.parentNode) {
            ...
            setTimeout(() => {
                ...
                console.log('Acorn removed, remaining:', acorns.length);
            }, 300);
        }
    }, 15000);
}